...
const REPO_NAME = REPO_NAME_ELEM.attributes
  .getNamedItem("href")
  .value.substr(1);
...
function fetchAndInjectBadge() {
  if (document.getElementById(SECARTA_BADGE_ID) == null) {
    getBrowserStorage(["access_token", "expires_at"])
      .then(storage => {
        const expiresAt = storage["expires_at"];
        if (storage["access_token"] == null || expiresAt == null) {
      ...
      .then(storage => {
        return fetchRepo(REPO_NAME, storage["access_token"]);
      })
...
function buildButton(response) {
  ...
  link.setAttribute("href", buildReportLinkForRepo(REPO_NAME));
